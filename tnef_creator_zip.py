#!/usr/bin/env python3
# TNEF Creator Zip - Generate TNEF file with multiple files embedded in a ZIP archive
# This script creates a TNEF file (winmail.dat) with a ZIP archive containing multiple files
...
def create_zip_file(attachments, zip_filename="attachments.zip"):
    """Create a ZIP file containing the specified attachments
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for attachment in attachments:
                ...
                zipf.write(file_path, attachment['filename'])
...
def create_tnef_file(output_file, attachments, zip_filename="attachments.zip"):
    """Create a TNEF file with a ZIP archive containing multiple files
    
    Args:
        output_file (str): Path to the output TNEF file
        attachments (list): List of dictionaries with 'filename' and 'content' keys
        zip_filename (str): Name of the ZIP file to embed
    """
    # Create a ZIP file containing the attachments
    zip_content = create_zip_file(attachments, zip_filename)
    ...
    # Body
    body = b"This is a TNEF message with a ZIP archive containing multiple files.\x00"
    ...
    filename_bytes = zip_filename.encode('utf-8') + b'\x00'
    tnef_data += create_tnef_attribute(LVL_ATTACHMENT, attAttachTitle, atpString, filename_bytes)
    
    # 3. Transport Filename
    tnef_data += create_tnef_attribute(LVL_ATTACHMENT, attAttachTransportFilename, atpString, filename_bytes)
    
    # 4. Attachment Creation Date
    tnef_data += create_tnef_attribute(LVL_ATTACHMENT, attAttachCreateDate, atpDate, date_sent)
    
    # 5. Attachment Modification Date
    tnef_data += create_tnef_attribute(LVL_ATTACHMENT, attAttachModifyDate, atpDate, date_sent)
    
    # 6. Attachment Data
    tnef_data += create_tnef_attribute(LVL_ATTACHMENT, attAttachData, atpByte, zip_content)
    
    # 7. End of this attachment
    ...
    print(f"Embedded ZIP file: {zip_filename} ({len(zip_content)} bytes)")
    print(f"Number of files in ZIP: {len(attachments)}")
    for i, attachment in enumerate(attachments, 1):
        print(f"  {i}. {attachment['filename']} ({len(attachment['content'])} bytes)")
    print(f"Total TNEF file size: {len(tnef_data)} bytes")
...
def main():
    parser = argparse.ArgumentParser(description='Create a TNEF file with a ZIP archive containing multiple files')
    parser.add_argument('-o', '--output', default='winmail.dat', help='Output TNEF file (default: winmail.dat)')
    parser.add_argument('-z', '--zip-name', default='attachments.zip', help='Name of the embedded ZIP file (default: attachments.zip)')
    ...
    create_tnef_file(args.output, attachments, args.zip_name)
