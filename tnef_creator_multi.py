#!/usr/bin/env python3
# TNEF Creator Multi - Generate TNEF files with multiple custom embedded filenames
...
def add_attachment_attributes(tnef_data, filename, file_content):
    """Add attachment attributes for a single file to the TNEF data"""
    ...
    # Empty attachment marker (used by some TNEF parsers)
    tnef_data += create_tnef_attribute(LVL_ATTACHMENT, attAttachment, atpByte, b'')
    
    return tnef_data

def create_tnef_file(output_file, attachments):
    ...
    # Add attachment-level attributes for each file
    for attachment in attachments:
        tnef_data = add_attachment_attributes(tnef_data, attachment['filename'], attachment['content'])
...
def main():
    ...
    create_tnef_file(args.output, attachments)
