#!/usr/bin/env python3
# TNEF Creator Multi V3 - Generate TNEF files with multiple custom embedded filenames
# This script creates a TNEF file (winmail.dat) with multiple custom embedded files
# Fixed version based on detailed analysis of TNEF format and tnef tool source code
...
def create_tnef_file_with_multiple_attachments(output_file, attachments):
    ...
    for i, attachment in enumerate(attachments):
        ...
        transport_filename = f"attachment{i+1}.dat\x00".encode('utf-8')
        
        # 1. Start with Attachment Rendering Data
        # This is a critical attribute for TNEF compatibility
        # Format: WORD (attachment method), DWORD (attachment position), DWORD (render flags)
        ...
        # 7. Add MAPI Properties (dummy data to ensure length > 4)
        # This is critical for compatibility with the tnef tool
        # The mapi_attr_read function requires len > 4
        # Format: DWORD (number of properties), followed by property data
        mapi_data = struct.pack("<I", 0)  # 0 properties, but length is 4 bytes
        # Add padding to ensure length > 4
        mapi_data += b"\x00\x00\x00\x00\x00"  # 5 bytes of padding
        tnef_data += create_tnef_attribute(LVL_ATTACHMENT, attMAPIProps, atpByte, mapi_data)
        
        # 8. Add Attachment marker to indicate end of this attachment
        # This is a critical step for compatibility with standard TNEF tools
        # The attachment marker must have a non-empty data field with length > 4
        tnef_data += create_tnef_attribute(LVL_ATTACHMENT, attAttachment, atpDword, struct.pack("<I", 0) + b"\x00\x00\x00\x00\x00")
    
    # Add End of Attachments marker at the message level
    # This is required to properly terminate the TNEF file
    # Ensure length > 4 for compatibility with tnef tool
    tnef_data += create_tnef_attribute(LVL_MESSAGE, attEndOfAttachments, atpByte, b"\x00\x00\x00\x00\x00")
