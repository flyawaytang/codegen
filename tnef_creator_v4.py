#!/usr/bin/env python3
# TNEF Creator V4 - Generate TNEF files with custom embedded filenames
...
def get_current_time_as_tnef_date():
    """Convert current time to TNEF date format (14 bytes)"""
    now = datetime.datetime.now()
    # TNEF date format: year, month, day, hour, minute, second, dayofweek, 
    # plus padding to make it 14 bytes total
    year = now.year
    month = now.month
    day = now.day
    hour = now.hour
    ...
    # Pack as 7 16-bit values (14 bytes total)
    return struct.pack("<HHHHHHH", year, month, day, hour, minute, second, dayofweek)
...
def create_tnef_file(output_file, embedded_filename, file_content=None):
    ...
    # 5. Creation and Modification Dates (using TNEF date format)
    date_data = get_current_time_as_tnef_date()
