#!/usr/bin/env python3
"""
PDF File Embedder with Custom Object Numbers (Final Version)

This script embeds files into PDF documents with precise control over object numbers
and ensures the embedded file is placed at the end of the PDF.

Usage:
    python pdf_file_embedder_final_custom.py input.pdf file_to_embed output.pdf --obj-num 100 --filespec-obj-num 101
"""
...
def embed_file_with_custom_obj_num(input_pdf_path, file_to_embed_path, output_pdf_path, obj_num=None, filespec_obj_num=None, mime_type=None):
    """
    Embed a file into a PDF with custom object numbers.
    ...
    try:
        ...
        # Step 1: Create a new PDF with the same content as the input PDF
        ...
        # Step 3: Create a new PDF with our embedded file
        with Pdf.open(temp_pdf_path) as pdf:
            ...
            # Save the PDF with our embedded file
            temp_output_path = os.path.join(temp_dir, "temp_output.pdf")
            pdf.save(temp_output_path)
        
        # Step 4: Use qpdf to optimize the PDF and ensure our objects are at the end
        # This also helps ensure the PDF is valid
        qpdf_cmd = [
            "qpdf",
            "--object-streams=disable",  # Disable object streams for easier manipulation
            "--compress-streams=n",      # Disable stream compression for easier inspection
            "--linearize",               # Linearize the PDF (web-optimized)
            temp_output_path,
            output_pdf_path
        ]
        
        try:
            subprocess.run(qpdf_cmd, check=True, capture_output=True)
            print(f"Successfully embedded file with qpdf optimization")
        except subprocess.CalledProcessError as e:
            print(f"Warning: qpdf optimization failed: {e}")
            # Fall back to using the pikepdf output directly
            shutil.copy2(temp_output_path, output_pdf_path)
            print(f"Using pikepdf output directly")
        
        # Step 5: Verify the object numbers
        try:
                ...
                if '/Names' in pdf.Root and '/EmbeddedFiles' in pdf.Root.Names:
                    ...
                    if len(names_array) >= 2:
                        ...
                        if '/EF' in filespec and '/F' in filespec.EF:
                            ...
                            actual_embedded_file_num = get_object_number(embedded_file)
                            actual_filespec_num = get_object_number(filespec)
                            
                            print(f"Actual embedded file object number: {actual_embedded_file_num}")
                            print(f"Actual file specification object number: {actual_filespec_num}")
                            
                            return actual_embedded_file_num, actual_filespec_num
        except Exception as e:
            print(f"Warning: Could not verify object numbers: {e}")
        
        return embedded_file_obj_num, filespec_obj_num
...
def main():
    ...
    # Embed the file with custom object numbers
    print(f"\nEmbedding file: {args.file_to_embed}")
    try:
        embedded_file_obj_num, filespec_obj_num = embed_file_with_custom_obj_num(
            args.input_pdf,
            args.file_to_embed,
            args.output_pdf,
            args.obj_num,
